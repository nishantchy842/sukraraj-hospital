...
export default function NoticeCategorySideBar({
         ...
         <section className='max-h-[800px] overflow-y-auto'>
            <article className='flex w-[308px] flex-col gap-y-[10px] md:w-full md:flex-row md:flex-wrap md:gap-x-[15px] sm:hidden'>
               {data?.result.map((item) => (
                  <Link
                     key={item.id}
                     href={`${pathname === conditionalPath ? item.slug : `notices/${item.slug}`}`}
                  >
                     <div
                        className='flex w-full items-center justify-between px-[15px] py-[10px] md:gap-x-[20px]'
                        style={{
                           background:
                              item.slug === decodeURIComponent(category)
                                 ? '#0C62BB'
                                 : '#0C62BB0D',
                        }}
                     >
                        ...
                        >
                           {lang === 'en'
                              ...
                              : (item.title_Np ?? item.title_En).toUpperCase()}
                        </p>
                        <svg
                           width='9'
                           height='14'
