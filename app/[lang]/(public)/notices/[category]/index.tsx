'use client';
import { getAllNotice, getSingleNoticeCategory } from '@/api/notice';
import { useQuery } from '@tanstack/react-query';
import React, { useState } from 'react';
import { noticeConfig } from './config';
import { queryKeys } from '@/utils';
import { NOTICE, NOTICE_CATEGORY_DETAILS } from '@/constants/querykeys';
...
import NoticeCategorySideBar from './@sidebar';
...
export default function Notice({
   ...
   const conditionalPath = `/${lang}/notices/${category}`;
      ...
      <div className=' sm:pb-[40px]'>
         ...
         <section className='px-[60px] md:px-[50px] sm:px-[20px]'>
            <article className='mx-auto mt-[30px] flex max-w-[1440px] gap-x-[31px] md:flex-col  sm:mb-[40px]  sm:mt-[15px] sm:flex-col '>
               <NoticeCategorySideBar params={{ lang, category }} />
               {isLoading ? (
                  <Skeleton active />
               ) : (
                  <div className='w-full md:mt-[30px] sm:mb-[30px]'>
                     {isEmpty(data?.result) ? (
                        ...
                        <article className=' w-full border '>
                           {data?.result?.map((notice) => (
                              ...
                              >
                                 <div className='border-r  pl-[27px] pr-[25px] sm:px-[15px]'>
                                 ...
                                 <div className='flex flex-auto items-center justify-between  sm:flex-1'>
                                    <Space direction='vertical' size={7}>
                                       <p className=' text-[18px] font-medium leading-[25px] text-grey30'>
                                          {lang === 'en'
                           ...
                           {pathname !== conditionalPath && (
                              <div className='py-[19px] pl-[22px]'>
                                 <button
                                    type='button'
                                    className=' flex items-center justify-between gap-x-[7px] rounded-[5px] bg-primary px-[20px] py-[11.5px] text-[16px] font-medium leading-[26px] text-white'
                                    onClick={() => {
                                       router.push(
                                          `/${lang}/notices/${category}`
                                       );
                                    }}
                                 >
                                    <span>View more</span>
                     ...
                     {pathname === conditionalPath && (
                        <section className='mx-auto mt-[23px] flex max-w-[1440px] items-center justify-end  sm:my-[20px] sm:pb-0'>
                           <Pagination
                              current={config?.page}
